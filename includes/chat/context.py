...
class ChatContext(Protocol):
    ...
    # Open an interactive form card in the conversation (see
    # ``includes/chat/widgets.py``). Returns the id of the step that renders it;
    # the card's own state is persisted on that step, so the returned id is what
    # later re-renders and submissions address.
