"""Reusable country, state, and currency reference data for vendor forms.

NetSuite country internal IDs ARE the 2-letter ISO codes (verified live),
so these lists double as the id source for the address country ref.
"""

#: (ISO code, display name) — ISO 3166-1, ordered by name.
COUNTRIES: list[tuple[str, str]] = [
    ...
    ("CG", "Congo (Brazzaville)"), ("CD", "Congo (Kinshasa)"),
    ...
    ("KR", "Korea (South)"), ("KW", "Kuwait"), ("KG", "Kyrgyzstan"),
    ("LA", "Laos"), ("LV", "Latvia"), ("LB", "Lebanon"),
    ("LS", "Lesotho"), ("LR", "Liberia"), ("LY", "Libya"),
    ("LI", "Liechtenstein"), ("LT", "Lithuania"), ("LU", "Luxembourg"),
    ("MO", "Macau"), ("MG", "Madagascar"), ("MW", "Malawi"),
...
#: Australian states/territories for the state dropdown.
AU_STATES: list[str] = ["NSW", "VIC", "QLD", "SA", "WA", "TAS", "NT", "ACT"]

#: Country ISO code → currency symbol (basic mapping for form defaults).
COUNTRY_CURRENCY: dict[str, str] = {
    "AU": "AUD", "NZ": "NZD", "US": "USD", "GB": "GBP", "CA": "CAD",
    "SG": "SGD", "JP": "JPY", "ZA": "ZAR", "PH": "PHP", "HK": "USD",
    "AT": "EUR", "BE": "EUR", "CY": "EUR", "DE": "EUR", "ES": "EUR",
    "FI": "EUR", "FR": "EUR", "GR": "EUR", "IE": "EUR", "IT": "EUR",
    "LT": "EUR", "LU": "EUR", "LV": "EUR", "MT": "EUR", "NL": "EUR",
    "PT": "EUR", "SI": "EUR", "SK": "EUR", "EE": "EUR", "MC": "EUR",
    "SM": "EUR", "VA": "EUR",
}


#: Countries shown first in the dropdown ("Common" group).
COMMON_COUNTRY_CODES: list[str] = [
    "AU", "US", "GB", "NZ", "CA", "SG", "JP", "ZA", "PH",
]


def country_option_groups() -> list[dict]:
    """Country dropdown groups: common trading countries first.

    The countries in ``COMMON_COUNTRY_CODES`` lead the list in a
    "Common" group; everything else follows alphabetically.
    """
    common_codes = set(COMMON_COUNTRY_CODES)
    common = sorted(
        ({"code": c, "name": n} for c, n in COUNTRIES if c in common_codes),
        key=lambda o: o["name"],
    )
    rest = [{"code": c, "name": n} for c, n in COUNTRIES if c not in common_codes]
    return [
        {"label": "Common", "options": common},
        {"label": "All countries", "options": rest},
    ]
