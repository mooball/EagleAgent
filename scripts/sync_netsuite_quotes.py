"""
sync_netsuite_quotes.py

Syncs Quote (Estimate) line items from NetSuite into the local transactions table.
...
For each line item:
  ...
  - If new: insert a new Transaction record with doc_type='Quote'
  - Lines without a matching product or supplier are skipped (logged)

Sort ASC by lastmodifieddate so --resume always makes forward progress.

Usage:
  uv run python -m scripts.sync_netsuite_quotes
  uv run python -m scripts.sync_netsuite_quotes --since 2026-04-01
  uv run python -m scripts.sync_netsuite_quotes --since 7d
  uv run python -m scripts.sync_netsuite_quotes --resume
  uv run python -m scripts.sync_netsuite_quotes --dry-run
"""
...
def main():
    parser = argparse.ArgumentParser(description="Sync Quote lines from NetSuite into the database.")
    parser.add_argument(
        "--since",
        type=str,
        default=None,
    ...
    parser.add_argument(
        "--resume",
        action="store_true",
        help="Resume from last synced position (uses MAX netsuite_last_modified for Quote rows).",
    )
    ...
    if args.resume:
        ...
        if max_date:
            ...
            print("No existing Quote transactions found. Starting full sync from 2014-01-01.")
    elif args.since:
    ...
    query = quotes_updated_since(since_date)
    print(f"Fetching Quote lines modified since {since_date} (sorted oldest first)...")
