"""
import_purchase_history.py

Imports purchase history CSV data from the designated IMPORT_DIR into the
product_suppliers table. Matches part_number → products.id and
supplier → suppliers.id using pre-built lookup caches.

Usage:
  Local database import:
    uv run python -m scripts.import_purchase_history

  Production database import:
    uv run python -m scripts.import_purchase_history --production
...
def strip_key(val: str) -> str:
    """Strip all formatting characters for fuzzy matching: remove spaces, dashes, hashes, etc."""
...
def build_product_lookup(engine):
    """Build exact and stripped product lookups from all products."""
    session = make_session(engine)
    try:
        products = session.query(Product.id, Product.part_number).all()
        exact_lookup = {}
        stripped_lookup = {}
        for pid, pn in products:
            if pn:
                exact_key = normalize_key(pn)
                if exact_key not in exact_lookup:
                    exact_lookup[exact_key] = pid
                sk = strip_key(pn)
                if sk not in stripped_lookup:
                    stripped_lookup[sk] = pid
        print(f"  Product lookup: {len(exact_lookup)} exact, {len(stripped_lookup)} stripped keys.")
...
def import_purchase_history(engine, df: pd.DataFrame, product_exact: dict, product_stripped: dict, supplier_lookup: dict, existing_keys: set):
    ...
    for i in range(0, len(df), BATCH_SIZE):
        ...
        try:
            ...
            for _, row in batch_df.iterrows():
                ...
                # Resolve product_id: exact match first, then stripped fallback
                product_id = product_exact.get(normalize_key(part_number))
                if not product_id:
                    product_id = product_stripped.get(strip_key(part_number))
                if not product_id:
                    unmatched_parts.add(part_number)
                    failed_rows.append({**row, '_reason': 'unmatched part_number'})
                    skipped += 1
                    continue
    ...
    if unmatched_parts:
        ...
        print(f"\n  Unmatched part numbers ({len(sorted_parts)}):")
        for pn in sorted_parts[:50]:
            print(f"    - {pn}")
        if len(sorted_parts) > 50:
            print(f"    ... and {len(sorted_parts) - 50} more.")
...
def main():
    parser = argparse.ArgumentParser(description="Import purchase history data into the database.")
    ...
    csv_files = glob.glob(os.path.join(import_dir, "purchase_history_import*.csv"))
    if not csv_files:
        print(f"No files matching 'purchase_history_import*.csv' found in {import_dir}.")
        return
    ...
    for filepath in csv_files:
        ...
        if failed_rows:
            failed_path = os.path.join(import_dir, "purchase_history_failed.csv")
